...
public class SVNLogRunner {
    ...
    public void runCommand(SVNAdminArea adminArea, String name, SVNProperties attributes, int count) throws SVNException {
        ...
        SVNDebugLog.getDefaultLog().log(SVNLogType.DEFAULT, "[RUN]" + name + " for admin dir: " + adminArea.getAdminDirectory() + " | fileName " + fileName + " | force = " + SVNProperty.booleanValue(attributes.getStringValue(SVNLog.FORCE_ATTR)), Level.FINEST);
        if (SVNLog.DELETE_ENTRY.equals(name)) {
            ...
            SVNDebugLog.getDefaultLog().log(SVNLogType.DEFAULT, "[RUN] move src = " + src.getPath() + " dst = " + dst.getPath(), Level.FINEST);
            try {
                SVNFileUtil.rename(src, dst);
            } catch (SVNException svne) {
                error = new SVNException(svne.getErrorMessage().wrap("Can't move source to dest"), svne);
            }
            SVNDebugLog.getDefaultLog().log(SVNLogType.DEFAULT, "[RUN] after move src length = " + src.length() + " dst length = " + dst.length(), Level.FINEST);
            ...
            SVNDebugLog.getDefaultLog().log(SVNLogType.DEFAULT, "[RUN] cp and translate src = " + src.getPath() + " dst = " + dst.getPath(), Level.FINEST);
            ...
            try {
                ...
                SVNDebugLog.getDefaultLog().log(SVNLogType.DEFAULT, "[RUN] after cp and translate src length = " + src.length() + " dst length = " + dst.length(), Level.FINEST);
            ...
            String versionedName = attributes.getStringValue(SVNLog.ATTR2);
            adminArea.closeVersionedProperties();
            SVNDebugLog.getDefaultLog().log(SVNLogType.DEFAULT, "[RUN] cp and detranslate src = " + fileName + " dst = " + dstName, Level.FINEST);
            try {
                SVNTranslator.translate(adminArea, versionedName != null ? versionedName : fileName, fileName, dstName, false);
            } catch (SVNException svne) {
                error = svne;
            }
            SVNDebugLog.getDefaultLog().log(SVNLogType.DEFAULT, "[RUN] after cp and detranslate src length = " + adminArea.getFile(fileName).length() + " dst length = " + adminArea.getFile(dstName).length(), Level.FINEST);
        } else if (SVNLog.COPY.equals(name)) {
            File src = adminArea.getFile(fileName);
            File dst = adminArea.getFile(attributes.getStringValue(SVNLog.DEST_ATTR));
            SVNDebugLog.getDefaultLog().log(SVNLogType.DEFAULT, "[RUN] copy src = " + src.getPath() + " dst = " + dst.getPath(), Level.FINEST);
            try {
                SVNFileUtil.copy(src, dst, true, false);
            } catch (SVNException svne) {
                error = svne;
            }
            SVNDebugLog.getDefaultLog().log(SVNLogType.DEFAULT, "[RUN] after copy src length = " + src.length() + " dst length = " + dst.length(), Level.FINEST);
            ...
            try {
                ...
                SVNDebugLog.getDefaultLog().log(SVNLogType.DEFAULT, "[RUN] merge local path = " + fileName + " left = " + leftPath + "right = " + rightPath, Level.FINEST);
                ...
                SVNDebugLog.getDefaultLog().log(SVNLogType.DEFAULT, "[RUN] after merge local path length = " + adminArea.getFile(fileName).length() + " left length = " + adminArea.getFile(leftPath).length() + "right length= " + adminArea.getFile(rightPath).length(), Level.FINEST);
    ...
    public void logFailed(SVNAdminArea adminArea) throws SVNException {
        ...
        SVNDebugLog.getDefaultLog().log(SVNLogType.DEFAULT, "[END] log failed for " + adminArea.getAdminDirectory(), Level.FINEST);
    }
    ...
    public void logCompleted(SVNAdminArea adminArea) throws SVNException {
        ...
        SVNDebugLog.getDefaultLog().log(SVNLogType.DEFAULT, "[END] log completed for " + adminArea.getAdminDirectory(), Level.FINEST);
    }
