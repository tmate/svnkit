 ...
 * @since 4.1
 */
final class NTLMEngine implements INTLMEngine {
    ...
    static {
        ...
        try {
            ...
            bytesWithoutNull = new byte[] {'N', 'T', 'L', 'M', 'S', 'S', 'P'};
        }
    ...
    static class NTLMMessage {
        ...
        NTLMMessage(final String messageBody, final int expectedType) throws NTLMEngineException {
            final byte[] buffer = new byte[messageBody.length()];
            int length = SVNBase64.base64ToByteArray(new StringBuffer(messageBody), buffer);
            messageContents = new byte[length];
            System.arraycopy(buffer, 0, messageContents, 0, length);
        ...
        String getResponse() {
            ...
            return SVNBase64.byteArrayToBase64(resp);
        }
