...
public class SVNUpdateClient16 extends SVNUpdateClient {
	protected SVNUpdateClient dispatcher;

	protected SVNUpdateClient16(SVNUpdateClient from) {
		super(from);
		this.dispatcher = dispatcher;
	}

	public static SVNUpdateClient16 delegate(SVNUpdateClient dispatcher) {
		SVNUpdateClient16 delegate = new SVNUpdateClient16(dispatcher);
		return delegate;
	}
	...
	public void copyVersionedFile(File dstPath, SVNAdminArea adminArea,
		...
		String charset = SVNTranslator.getCharset(charsetProp, adminArea
				.getFile(fileName).getPath(), getOptions());
	...
	public long doRemoteExport(SVNRepository repository, final long revNumber,
		...
		if (dstKind == SVNNodeKind.DIR) {
			...
			try {
				...
				boolean binary = SVNProperty.isBinaryMimeType(properties
						.getStringValue(SVNProperty.MIME_TYPE));
				String charset = SVNTranslator
						.getCharset(properties
								.getStringValue(SVNProperty.CHARSET), url,
								getOptions());
	...
	public long update(File path, SVNRevision revision, SVNDepth depth,
		...
		try {
			...
			final SVNRepository repos2 = createRepository(reposRoot, null,
					null, false);
			ISVNFileFetcher fileFetcher = new ISVNFileFetcher() {
				public long fetchFile(String path, long revision,
						OutputStream os, SVNProperties properties)
						throws SVNException {
