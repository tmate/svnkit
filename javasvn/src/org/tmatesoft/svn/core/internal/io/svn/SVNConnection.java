...
class SVNConnection {
    ...
    protected void handshake(SVNRepositoryImpl repository) throws SVNException {
        ...
        if (!SVNReader.hasValue(items, 0, 2)) {
            SVNException.throwException(SVNErrorMessage.create(SVNErrorCode.RA_SVN_BAD_VERSION, "Only protocol of version '2' or older is supported"));
        } else if (!SVNReader.hasValue(items, 2, EDIT_PIPELINE)) {
            SVNException.throwException(SVNErrorMessage.create(SVNErrorCode.RA_SVN_BAD_VERSION, "Only servers with 'edit-pipeline' capability is supported"));
        }
    ...
    public void authenticate(SVNRepositoryImpl repository) throws SVNException {
        ...
        if (repository.getExternalUserName() != null && mechs.contains("EXTERNAL")) {
            ...
            SVNException.throwException(SVNErrorMessage.create(SVNErrorCode.RA_UNKNOWN_AUTH));
        }
        if (failureReason == null) {
            return;
        }
        SVNException.throwException(failureReason);
    }
